...
`backend/test`; следующие этапы до `0132` находятся в `backend/work`, а
интерфейс этапов 3–10 — в `crm/work`, не в `crm/test`.
Фактическое применение ревизий в БД test/production не подтверждено;
в ходе этой сверки тесты и генераторы снимков не запускались.
...
`bonus_size` или вручную дублировать данные визита в `fields`. В work снимки
OpenAPI/schema статически содержат новые endpoints и таблицы до `0135`, но
их равенство runtime-контракту и БД не проверено. CRM gateway написан вручную:
его совместимые defaults для отсутствующих полей могут выглядеть как пустые
корректные значения старого API. Сверка DTO перед переносом обязательна.
