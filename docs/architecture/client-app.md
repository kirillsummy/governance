...
**Текущий Git-срез 25.09.2026:** доработки жалобы, записи с допуслугами,
любимых мастеров, лояльности и акций включены в `client-app/test`.
Клиентская жалоба показывает услугу, описание и фото после входа по телефону;
на локальном просмотре без сессии открывается экран входа. TEST получил
клиентское приложение до этого переноса Git-веток, но работа YClients на
стенде отключена; реальную запись и бонусный баланс сквозным сценарием
не проверяли. Ниже сохранён исторический путь `work`.
