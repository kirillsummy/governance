...
а не подтверждённая приёмка test/production: CRM/test отстаёт от work на десять
коммитов, не проверены миграции и сквозные
...
Gateway процессов — ручной adapter, не сгенерированный клиент OpenAPI;
совместимые defaults могут скрыть старое поле как пустое значение.
