...
в этом осмотре не проверялись. Запрещённые оператором проверки и реальные
вызовы не запускались.

| Область | Подтверждено в исходниках/ветках | Пока не подтверждено |
|---|---|---|
| Backend | `dev` содержит базовую линию до `0116`; `test` включает `0125` и заканчивается `0126`; `work` продолжена до `0135`, включая этапы рекламации 3–10 | Применение миграций в БД, совместимость разошедшихся `test`/`work`, реальные права и YClients |
| CRM | В `work` есть создание, карточка, задания, переходы, исполнение, переделка, штрафы и серверные фильтры рекламаций. `test` на 10 коммитов позади; `main` ещё старше | Сквозная приёмка, развернутый SHA; уведомления рекламаций отсутствуют |
| Master-app | `test`/`work` разошлись, дальнейшие фичи мастера развиваются отдельно | Экран рекламаций мастера не утверждён и не реализован |
...
`db/api-contract.json` и `db/schema-contract.json` есть в backend/work и
статически содержат новые поверхности/таблицы, но генерация и контрактные
проверки не запускались: полное соответствие текущему коду **не доказано**.
...
фиктивной карточкой. Backend outbox-worker обслуживает рекрутинг, не
адресные уведомления рекламаций.
...
Филиальные права, зависший `chosen` и отсутствие уведомлений — отдельные
блокеры. Неутверждённые продуктовые решения и конфликты перечислены в
...
или отправку уведомления автоматически. **Только локальный сбор Git-метаданных
первой строки выполнялся в этом осмотре**; серверные, контрактные и
автоматические проверки остальных строк не запускались.
